...
import { Search, Menu, User as UserIcon, Grid3X3, Bell, X } from 'lucide-react';
...
export default function Header({ onMobileMenuClick }: HeaderProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-slate-900/95 backdrop-blur-md border-b border-blue-500/30 transition-all shadow-lg">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 gap-2 sm:gap-4">
          ...
          <div className="hidden md:flex flex-1 justify-center max-w-md mx-4">
            <div className="relative w-full">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-slate-800 border border-blue-500/30 rounded-lg text-sm text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all duration-200 hover:bg-slate-700 min-h-[40px]"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
            {/* MakrX Apps Launcher - Hidden on mobile */}
            ...
            <Button
              ...
              size="sm"
              onClick={toggleTheme}
              className="hidden sm:flex p-2 rounded-lg hover:bg-blue-500/10 hover:shadow-lg transition-all text-blue-400"
            ...
            <Button
              ...
              className="md:hidden p-2 rounded-lg hover:bg-blue-500/10 hover:shadow-lg transition-all text-blue-400"
              aria-label="Search"
            >
            ...
            >
              ...
              <span className="absolute -top-1 -right-1 w-2.5 h-2.5 sm:w-3 sm:h-3 bg-red-500 rounded-full text-xs"></span>
            </Button>

            {/* User Menu */}
            <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-2 bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/30 rounded-lg shadow-lg backdrop-blur-sm">
              <UserIcon className="w-4 h-4 text-blue-400 flex-shrink-0" />
              <div className="hidden sm:block min-w-0">
                <div className="text-sm font-medium text-white truncate">{formatUserDisplayName(user)}</div>
      ...
      {showMobileSearch && (
        <div className="md:hidden fixed inset-x-0 top-14 z-50 bg-slate-900/95 backdrop-blur-md border-b border-blue-500/30 p-4">
